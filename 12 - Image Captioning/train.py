...
from utils import clip_gradient, save_checkpoint, AverageMeter, adjust_learning_rate, accuracy
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # Print status
        print(
            f'\rEpoch: [{epoch}][{i}/{len(train_loader)}]\t'
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            print(
                f'\rValidation: [{i}/{len(val_loader)}]\t'
                f'Batch Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                f'Loss {losses.val:.4f} ({losses.avg:.4f})\t'
                f'Top-5 Accuracy {top5accs.val:.3f} ({top5accs.avg:.3f})\t',
                end='\r'
            ...
            for j in range(allcaps.shape[0]):
                ...
                img_captions = list(map(
                    lambda c: [w for w in c if w not in {word_map['<start>'], word_map['<pad>']}], img_caps
